...
stream_seasonal = data.frame(stream) %>%
  mutate(year = year(DateTime)) %>%
  merge(snow_bounds, by = 'year') %>%
  mutate(season = ifelse(yday(DateTime) > lastNZ & yday(DateTime) < 183, 'Melt', ifelse(yday(DateTime) > 182 & yday(DateTime) < 289, 'Growing', 'Snow'))) %>%
  group_by(season, year) %>%
...
print(head(highSnow_seasonal))
print(head(lowSnow_seasonal))
```
...
calc_seasonalQ_contribution_Conditional = function(var, lag, data){
    ...
    for(i in unique(data$season)){
        ...
        temp = append(temp, natstobits(condinformation(X = discretize(lagDat_seasonal$shiftedX),
...
calc_seasonalQ_correlations = function(var, lag, data){
  ...
  for(i in unique(data$season)){
    ...
    #correlation
    temp = append(temp, cor(x = lagDat_seasonal$shiftedX, y = lagDat_seasonal$shiftedY))
...
for(lag in seq(1, 2)){
  ...
  for(v in vars){
    seasonal_MI_data[v] = calc_seasonalQ_contribution_Conditional(v, lag, data_seasonal)
    seasonal_R_data[v] = calc_seasonalQ_correlations(v, lag, data_seasonal)
  }
  
  #Plot stacked time series (plus shifted data)
  x  = seq(1:3)
  seasons = c(x[(length(x)-lag+1):length(x)], x[1:(length(x)-lag)])
  seasonal_MI_data = seasonal_MI_data %>% 
    mutate(season = seasons) %>%
    gather(variable, value, vars)
  
  #1 - streamflow, 2 - frost, 3 - melt
  monthlyplot = ggplot(data = seasonal_MI_data, aes(x = season, y = value, fill = variable)) +
    #Plot
  ...
  monthlyplot_corr = ggplot(data = seasonal_R_data, aes(x = season, y = value, col = variable)) +
    geom_line() +
    geom_point() + 
...
for(v in vars){
  ...
  for(lag in seq(1, 2)){
    temp = calc_seasonalQ_contribution_Conditional(v, lag, data_seasonal)
    temp_corr = calc_seasonalQ_correlations(v, lag, data_seasonal)
...
for(lag in seq(1, 2)){
  ...
  for(v in vars){
    seasonal_MI_data[v] = calc_seasonalQ_contribution_Conditional(v, lag, highSnow_seasonal)
    seasonal_R_data[v] = calc_seasonalQ_correlations(v, lag, highSnow_seasonal)
  }
...
for(lag in seq(1, 2)){
  ...
  #save
  save_plot(paste0('./Figures/highsnow_variableMI_lag', lag, '.pdf'), monthlyplot, base_height = 6, base_asp = 1.2)
...
for(lag in seq(1, 2)){
  ...
  for(v in vars){
    seasonal_MI_data[v] = calc_seasonalQ_contribution_Conditional(v, lag, lowSnow_seasonal)
    seasonal_R_data[v] = calc_seasonalQ_correlations(v, lag, lowSnow_seasonal)
  }
...
for(lag in seq(1, 2)){
  ...
  save_plot(paste0('./Figures/lowsnow_variableMI_lag', lag, '.pdf'), monthlyplot, base_height = 6, base_asp = 1.2)
