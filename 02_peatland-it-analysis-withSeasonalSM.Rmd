...
```{r}
write.csv(dat, './mef-data/data_compiled_gapfilled_seasonal.csv', row.names = FALSE)
dat
```
...
```{r}
#Average out the south and east soil moisture
data_seasonal['d.15'] = (data_seasonal['d.15_S2S'] + data_seasonal['d.15_S2E'])/2
data_seasonal['d.76'] = (data_seasonal['d.76_S2S'] + data_seasonal['d.76_S2E'])/2
data_seasonal['d.137'] = (data_seasonal['d.137_S2S'] + data_seasonal['d.137_S2E'])/2

data_seasonal = data_seasonal %>%
  select(-contains(c("S2S", "S2E")))
```
...
```{r}
#PLOTTING FUNCTIONS
vars = c( "d.15", "d.76", "d.137", "MEANC",  "South_PCP", "Snow_in", "WTE")
...
plotQuadratVar(var = 'WTE', ylab = 'Water Table Elevation [m]')
```
...
#Add years
frostUpland_merged = frostUpland %>%
  mutate(year = year(Date)) %>%
  merge(snowAnnual[c('year', 'typeS', 'totSnow')], on = 'year') %>% #Annual Snow Accumulation
  merge(annualPrecip[c('year', 'typeP', 'totP')], on = 'year') %>% #Annual Precip 
  merge(winterPrecip[c('year', 'typeP_snow', 'winterTotP')], on = 'year') %>% #Snow Season Precipitation
  merge(meltPrecip[c('year', 'typeP_melt', 'meltTotPrecip')], on = 'year') %>% #Melt Season Precipitation
  merge(growingPrecip[c('year', 'typeP_growing', 'growingTotP')], on = 'year') %>% #Growing Season Precipitation
  merge(winterTemp[c('year', 'typeT_snow', 'winterTempC')], on = 'year') %>% #Snow Season Temperature
  merge(meltTemp[c('year', 'typeT_melt', 'meltTempC')], on = 'year') %>% #Melt Season Temperature
  merge(growingTemp[c('year', 'typeT_growing', 'growingTempC')], on = 'year') %>% #Growing Season Temperature
  mutate(snowRatio = totSnow/totP)
...
```{r}
ggplot(data = frostUpland, aes(x = totSnow, y = winterTempC, col = FrostDEPTH, size = FrostDEPTH)) +
  geom_jitter(height = 0.5, width = 2) +
  geom_rug() +
  xlab('Total Annual Snowfall (in)') +
  ylab('Average Winter Temperature (C)') +
  theme_bw()

ggsave(filename = paste0('./Figures/VariablePlots/totSnow_winterTempC_FrostDepths.pdf'), height = 5 , width = 6)
```
...
print("High Frost")
print("-----------------------------")
plotMI_modified(data_seasonal,'High Frost', frostUpland_annual$year[frostUpland_annual$frostQuantMeas == 'High'])


print("Med Frost")
print("-----------------------------")
plotMI_modified(data_seasonal, 'Med Frost', frostUpland_annual$year[frostUpland_annual$frostQuantMeas == 'Med'])


print("Low Frost")
print("-----------------------------")
plotMI_modified(data_seasonal, 'Low Frost', frostUpland_annual$year[frostUpland_annual$frostQuantMeas == 'Low'])
```
